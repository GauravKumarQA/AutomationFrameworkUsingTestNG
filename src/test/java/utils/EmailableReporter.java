...
public class EmailableReporter implements IReporter {
  ...
  private void resultSummary(
    ...
    if (!tests.getAllResults().isEmpty()) {
      ...
      for (ITestNGMethod method : getMethodSet(tests, suite)) {
        ...
        String timeToPrint;
        long timeCheck =TimeUnit.MILLISECONDS.toSeconds(end - start);
        if(timeCheck<=60) {
        	timeToPrint=String.valueOf(timeCheck) + " sec";
        }
        else {

        	timeToPrint =	String.format("%02d min, %02d sec", 
  ...
  protected void generateExceptionReport(Throwable exception) {
    ...
    m_out.print(Utils.stackTrace(exception, true));
    m_out.println("</div>");
  }
  ...
  public void generateSuiteSummaryReport(List<ISuite> suites) {
    ...
    for (ISuite suite : suites) {
      ...
      for (ISuiteResult r : tests.values()) {
        ...
        String summaryIndividual ;
        if((TimeUnit.MILLISECONDS.toSeconds(overview.getEndDate().getTime() - overview.getStartDate().getTime()))<60) {
        	summaryIndividual = String.format("%02d sec", 
        			TimeUnit.MILLISECONDS.toSeconds(overview.getEndDate().getTime() - overview.getStartDate().getTime()) 
        			);
        }
        else {
        	summaryIndividual = String.format("%02d min, %02d sec", 
        			TimeUnit.MILLISECONDS.toMinutes(overview.getEndDate().getTime() - overview.getStartDate().getTime()),
        			TimeUnit.MILLISECONDS.toSeconds(overview.getEndDate().getTime() - overview.getStartDate().getTime()) - 
        			TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(overview.getEndDate().getTime() - overview.getStartDate().getTime()))
        			);
        }
        
        summaryCell(summaryIndividual, true);
    ...
    if (qty_tests > 1) {
      ...
      if((TimeUnit.MILLISECONDS.toSeconds(time_end - time_start))<60) {
    	  totalTimeValue=  String.format("%02d sec", 
      			TimeUnit.MILLISECONDS.toSeconds(time_end - time_start) 
      			);
      }
      else {
    	  totalTimeValue =  String.format("%02d min, %02d sec", 
      			TimeUnit.MILLISECONDS.toMinutes(time_end - time_start),
      			TimeUnit.MILLISECONDS.toSeconds(time_end - time_start) - 
      			TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(time_end - time_start))
      			);
      }
      summaryCell(totalTimeValue, true);
      //summaryCell(formatter.format((time_end - time_start) / 1000.) + " secondsTest2", true);
  ...
  protected void startHtml(PrintWriter out) {
    ...
    out.println("td,th {border:1px sol"
    		+ "id #009;padding:.25em .5em}");
